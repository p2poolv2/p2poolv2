...
impl Node {
    ...
    /// Returns true if the node should continue running, false if the node should shutdown
    pub fn handle_command(&mut self, command: Command) -> Result<bool, Box<dyn std::error::Error>> {
        match command {
            Command::SendShare(share) => {
                // self.swarm.behaviour_mut().send_share(share);
                Ok(true)
            }
            Command::Shutdown => {
