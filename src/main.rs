...
impl Node {
    ...
    pub fn send_to_peer(&mut self, peer_id: libp2p::PeerId, buf: Vec<u8>) {
        // if let Err(e) = self.swarm.behaviour_mut().request_response.send_request(&peer_id, buf) {
    ...
    fn handle_request_response_event(&mut self, event: RequestResponseEvent<InventoryMessage, InventoryMessage>) {
