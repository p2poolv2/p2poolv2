...
First build CKPool's https://bitbucket.org/ckolivas/ckpool/src/master/[ckpool-solo] branch.
Then run the CKPool stratum server using the provided configuration file. The configuration in `ckpool-testnet4-solo.json` points the mock bitcoind at `127.0.0.1:48332` and listens for stratum connections on `0.0.0.0:3333`.
...
Run jmeter from the `load-tests/jmeter-testing` directory. Use a different output file name for each test run to keep results separate.
...
The `benchmark.sh` script automates the full comparison between default builds, native builds, and CKPool. It starts mock-bitcoind, runs each variant with jmeter, and prints a summary with latency percentiles.
