...
//TODO config file has to be made much better. As of now i am using default values for most of the fields.
...
async fn make_node(node_config: NodeConfig)->ldk_node::Node{
    ...
    let _ = builder.set_listening_addresses(vec![
        node_config.listening_addresses.parse().unwrap_or_else(|_| {
            panic!("Invalid listening address: {}", node_config.listening_addresses)
        })
    ]);
...
async fn main(){
    ...
    let node_config = parse_config("config.toml")
