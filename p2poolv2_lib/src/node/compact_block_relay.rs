...
impl Encodable for ShareHeaderAndShortIds {
    fn consensus_encode<W: bitcoin::io::Write + ?Sized>(
        ...
        len += self.sharechain_header.consensus_encode(writer)?;
...
pub enum CompactBlockRelayMode {
    #[default]
    /// Handshaking
    Handshake = -1,
...
impl Display for CompactBlockRelayMode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CompactBlockRelayMode::Handshake => write!(f, "handshake"),
            CompactBlockRelayMode::LowBandwidth => write!(f, "low-bandwidth"),
            CompactBlockRelayMode::HighBandwidth => write!(f, "high-bandwidth"),
        }
    }
}
...
mod tests {
    ...
    fn test_compact_block_relay_status_default() {
        let status = CompactBlockRelayMode::default();
        assert_eq!(status, CompactBlockRelayMode::Handshake);
    }

    #[test]
    fn test_compact_block_serde() {
        let sharechain_header = TestShareBlockBuilder::new().build().header;
        let compact_block = ShareHeaderAndShortIds {
        ...
        let bin = consensus::encode::serialize(&compact_block);
        let decoded: ShareHeaderAndShortIds = consensus::encode::deserialize(&bin).unwrap();
        assert_eq!(compact_block, decoded);
    }
}
