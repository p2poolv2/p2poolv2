...
pub async fn send_initial_compact_relay<C: 'static>(
    ...
    if let Err(e) = swarm_tx.send(SwarmSend::Request(peer_id, message)).await {
        error!(
            "Failed to send {} sendcmpct message to peer {}: {}",
            mode_str, peer_id, e
        );
...
mod tests {
    ...
    async fn test_send_initial_compact_relay_high_bandwidth() {
        ...
        let result = send_initial_compact_relay(peer_id, 2, swarm_tx).await;
        assert!(result.is_ok());
    ...
    async fn test_send_initial_compact_relay_low_bandwidth() {
        ...
        let result = send_initial_compact_relay(peer_id, 3, swarm_tx).await;
        assert!(result.is_ok());
    ...
    async fn test_send_initial_compact_relay_channel_closed() {
        ...
        let result = send_initial_compact_relay(peer_id, 0, swarm_tx).await;
