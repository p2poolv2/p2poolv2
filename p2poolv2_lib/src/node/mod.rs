...
impl Node {
    ...
    async fn handle_identify_event(&mut self, event: identify::Event) {
        match event {
            identify::Event::Received { peer_id, info } => {
                ...
                // insert initial state for peer once established
                let _ = self.peer_states.get_or_insert(&peer_id);
    ...
    async fn handle_connection_established(
        ...
        let compact_block_peer_count = self
            .peer_states
            .all()
            .iter()
            .filter(|s| !matches!(s.compact_block_from, CompactBlockRelay::Disabled))
            .count();

        send_send_compact(peer_id, compact_block_peer_count as u8, &self.swarm_tx).await?;
