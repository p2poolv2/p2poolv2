...
/// ~1MiB
const CONNECTION_TRACKER_CHANNEL_SIZE: usize = const {
    const SIZE: usize = size_of::<ConnectionTrackerCommand>();
...
pub struct PeerInfo {
    ...
    pub compact_block_to: CompactBlockRelayMode,
    /// Whether we should expect compact blocks
    pub compact_block_from: CompactBlockRelayMode,
...
impl From<ConnectionTracker> for ConnectionTrackerActor {
    fn from(connection_tracker: ConnectionTracker) -> Self {
        ...
        let (ct_cmd_tx, ct_cmd_rx) = mpsc::channel(CONNECTION_TRACKER_CHANNEL_SIZE);
...
impl Drop for ConnectionTrackerActor {
    fn drop(&mut self) {
        // Graceful shutdown should already have been called
        // in Node.shutdown(). This just ensures it's done.
        self.join_handle.abort();
    }
}
...
impl ConnectionTracker {
    ...
    /// Count peers we connected to in HighBandwidth mode.
    pub(crate) fn count_high_bandwidth_peers_to(&self) -> u8 {
        self.connected_peers
            .values()
            .filter(|p| matches!(p.compact_block_to, CompactBlockRelayMode::HighBandwidth))
            .count() as u8
    }
    ...
    /// Set a peer's compact_block_to status. No-op (with debug log) if peer not tracked.
    pub(crate) fn set_compact_block_to(&mut self, peer_id: &PeerId, status: CompactBlockRelayMode) {
        match self.connected_peers.get_mut(peer_id) {
            Some(info) => info.compact_block_to = status,
            None => debug!(%peer_id, "set_compact_block_to: peer not tracked"),
        }
    }
            ...
            let (response, resp_tx) = match cmd {
                ...
                ConnectionTrackerCommand::CountHighBandwidthPeers { resp } => (
                    ConnectionTrackerResponse::HighBandwidthPeers(
                        tracker.count_high_bandwidth_peers_to(),
                    ),
                    resp,
                ),
                ...
                ConnectionTrackerCommand::SetCompactBlockTo(peer_id, status) => {
                    tracker.set_compact_block_to(&peer_id, status);
                    (ConnectionTrackerResponse::Ok, None)
                }
...
pub(crate) enum ConnectionTrackerCommand {
    ...
    SetCompactBlockFrom(PeerId, CompactBlockRelayMode),
    SetCompactBlockTo(PeerId, CompactBlockRelayMode),
    GetPeerInfos {
        resp: Option<oneshot::Sender<ConnectionTrackerResponse>>,
    },
    Shutdown,
...
impl ConnectionTrackerCommand {
    fn set_response_sender(&mut self) -> Option<oneshot::Receiver<ConnectionTrackerResponse>> {
        match self {
            ...
            | ConnectionTrackerCommand::SetCompactBlockTo { .. }
            | ConnectionTrackerCommand::Shutdown
...
impl ConnectionTrackerHandle {
    ...
    pub async fn set_compact_block_to(
        &self,
        peer_id: PeerId,
        status: CompactBlockRelayMode,
    ) -> Result<(), Box<dyn Error + Send + Sync>> {
        self.send_command(ConnectionTrackerCommand::SetCompactBlockTo(peer_id, status))
            .await
            .map(drop)
    }
...
mod tests {
    ...
    fn test_set_compact_block_from_tracked_peer() {
        ...
        tracker.set_compact_block_from(&peer_id, CompactBlockRelayMode::HighBandwidth);
        assert_eq!(
            tracker.connected_peers[&peer_id].compact_block_from,
    ...
    fn test_set_compact_block_from_untracked_peer_is_noop() {
        ...
        tracker.set_compact_block_from(&untracked, CompactBlockRelayMode::HighBandwidth);
        assert_eq!(tracker.connected_peers.len(), 0);
    }

    #[test]
    fn test_set_compact_block_to_tracked_peer() {
        let mut tracker = ConnectionTracker::new(HashSet::new());
        let peer_id = PeerId::random();
        tracker.handle_established(peer_id, &make_dialer_endpoint("/ip4/1.2.3.4/tcp/46884"));

        assert_eq!(
            tracker.connected_peers[&peer_id].compact_block_to,
            CompactBlockRelayMode::Handshake
        );

        tracker.set_compact_block_to(&peer_id, CompactBlockRelayMode::HighBandwidth);
        assert_eq!(
            tracker.connected_peers[&peer_id].compact_block_to,
            CompactBlockRelayMode::HighBandwidth
        );
    ...
    #[test]
    fn test_set_compact_block_to_untracked_peer_is_noop() {
        let mut tracker = ConnectionTracker::new(HashSet::new());
        let untracked = PeerId::random();
        tracker.set_compact_block_to(&untracked, CompactBlockRelayMode::HighBandwidth);
        assert_eq!(tracker.connected_peers.len(), 0);
    }

    #[test]
    fn test_count_high_bandwidth_peers_to() {
        let mut tracker = ConnectionTracker::new(HashSet::new());
        let peer_a = PeerId::random();
        let peer_b = PeerId::random();
        ...
        assert_eq!(tracker.count_high_bandwidth_peers_to(), 0);

        tracker.set_compact_block_to(&peer_a, CompactBlockRelayMode::HighBandwidth);
        assert_eq!(tracker.count_high_bandwidth_peers_to(), 1);

        tracker.set_compact_block_to(&peer_b, CompactBlockRelayMode::HighBandwidth);
        assert_eq!(tracker.count_high_bandwidth_peers_to(), 2);

        // LowBandwidth should not count
        tracker.set_compact_block_to(&peer_c, CompactBlockRelayMode::LowBandwidth);
        assert_eq!(tracker.count_high_bandwidth_peers_to(), 2);
    }

    #[tokio::test]
    async fn test_process_commands_set_compact_block_to_and_count() {
        let tracker = ConnectionTracker::new(HashSet::new());
        let (cmd_tx, cmd_rx) = mpsc::channel(CONNECTION_TRACKER_CHANNEL_SIZE);
        ...
        cmd_tx
            .send(ConnectionTrackerCommand::SetCompactBlockTo(
                peer_id,
                CompactBlockRelayMode::HighBandwidth,
            ))
            .await
        ...
        let (resp_tx, resp_rx) = oneshot::channel();
        cmd_tx
            .send(ConnectionTrackerCommand::CountHighBandwidthPeers {
                resp: Some(resp_tx),
            })
            .await
            .unwrap();
        match resp_rx.await {
            Ok(ConnectionTrackerResponse::HighBandwidthPeers(count)) => {
                assert_eq!(count, 1);
            }
            other => panic!("expected HighBandwidthPeers, got {:?}", other),
        }
    }
