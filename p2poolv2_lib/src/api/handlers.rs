...
fn open_secondary_db(path: &str, secondary_path: &str) -> Result<Arc<DB>, (StatusCode, Json<ErrorResponse>)> {
...
pub async fn get_shares(
    ...
    let db = open_secondary_db(
        "./store.db",
        "/tmp/rocksdb_api_secondary"
    )?;
    ...
    for (_key, value) in db.iterator_cf(cf, rocksdb::IteratorMode::Start).flatten() {
        if shares.len() >= params.limit.unwrap_or(100) { break; }
...
pub async fn get_pplns_distribution(
    ...
    let compact_target = bitcoin::pow::CompactTarget::from_unprefixed_hex(&template.bits)
        .map_err(|e| {
    ...
    let distribution = payout
        .get_output_distribution(&state.chain_store, total_difficulty, total_amount, &state.config)
        .await
        .map_err(|e| {
...
pub async fn health_check() -> Result<Json<HashMap<String, String>>, (StatusCode, Json<ErrorResponse>)> {
